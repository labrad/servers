...
class DacRunner(object):
    ...
    def extract(self, packets):
        """Extract timing data coming back from a readPacket."""
        #If number of packets received is less than the number expected,
        #print out which packet tags are missing
        if len(packets) != self.nPackets:
            data = ''.join(data[1:3] for data in packets)
            packetsReceived = set(np.fromstring(data, dtype='<u2').astype('u4'))
            packetsExpected = set(range(self.nPackets))
            packetsLost = packetsExpected - packetsReceived
            if packetsLost != set():
                print packetsLost
