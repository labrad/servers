...
class RegistryWrapperAsync(SafeIterDict):
    ...
    def _listen(self, listen):
        ...
        source = self._cxn.registry.ID
        func(self._messageReceived, source=source, context=self._ctx, ID=self.MESSAGE_ID)
    
    def _wrapSubdir(self, name):
        """Create a registry wrapper of a subdirectory."""
        return RegistryWrapper.create(self._cxn, self._dir + [d])
