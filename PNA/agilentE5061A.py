...
### BEGIN NODE INFO
[info]
name = Agilent E5061 ENA
version = 1.0
description = Agilent ENA 300kHz-1.5GHz Network Analyzer
...
class ENAWrapper(GPIBDeviceWrapper):
    @inlineCallbacks
    def initialize(self):
        yield self.write('FORM:DATA REAL')
        yield self.setupMeasurements(['S21'])

    @inlineCallbacks
    def setupMeasurements(self, desired_meas):
        if len(desired_meas)>1:
            raise Exception('Can only read out 1 S-parameter at a time.')
            
        resp = yield self.query('CALC:PAR:DEF?')
        
        if desired_meas != resp:
            yield self.write('CALC:PAR:DEF %s' %desired_meas[0])

class AgilentENAServer(GPIBManagedServer):
    name = 'Agilent E5061A ENA'
    deviceName = 'Agilent Technologies E5061A'
    deviceWrapper = ENAWrapper
    ...
    def getData(self, dev, meas):
        ...
        """
        yield dev.write("CALC:PAR:SEL")
        yield dev.write("CALC:DATA:SDAT?")
