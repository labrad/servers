...
### BEGIN NODE INFO
[info]
name = Agilent 1GHz DSO-X 4014A Oscilloscope
version = 0.2
description = Talks to the Agilent DSO-X 4014A 1GHz oscilloscope
...
class AgilentDSOX4014AServer(GPIBManagedServer):
    name = 'AGILENT DSO-X 4014A OSCILLOSCOPE'
    deviceName = 'AGILENT TECHNOLOGIES DSO-X 4104A'
    ...
    def scale(self, c, channel, scale = None):
        ...
        if scale is None:
            ...
            resp = yield dev.query('CHAN%d:SCAL?' %channel)
        scale = float(resp)*labrad.units.V
        returnValue(scale)
    ...
    def horiz_scale(self, c, scale = None):
        ...
        if scale is None:
            ...
            scale = format(scale,'E')
            yield dev.write('TIM:SCAL '+scale['s'])
            resp = yield dev.query('TIM:SCAL?')
        scale = float(resp)*labrad.units.s
        returnValue(scale)
    ...
    def get_trace(self, c, channel):
        ...
        traceVolts = ((trace*float(preambleDict['yStep'])+float(preambleDict['yOrigin'])))*labrad.units.V
        numPoints = int(preambleDict['numPoints'])
        time = numpy.linspace(float(preambleDict['xFirst']), (numPoints-1) * float(preambleDict['xStep'])+float(preambleDict['xFirst']),numPoints)*labrad.units.s
...
__server__ = AgilentDSOX4014AServer()
