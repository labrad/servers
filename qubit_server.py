...
class QubitServer(LabradServer):
    ...
    def new_expt(self, c, setup):
        ...
        for qindex, qname in enumerate(Setup['Qubits']):
            ...
            for ch, info in q['FOs'].items():
                ...
                expt['FOs'][ch, qindex] = {'FPGA': fpgaboard[1], 'FO': fo_channel[0]}
    ...
    def set_anritsu(self, c, data):
        ...
        if len(data) == 3:
            ...
            data = (T.Value(round(float(frq),5), 'GHz'), amp)
        else:
            channel = data
            data = None
    ...
    @setting(102, 'Experiment Turn Off Deconvolution',
                  channels=["(sw): Don't deconvolve this channel",
...
#                    print ch, ':', expt['Settlings'][ch]
    ...
    def run_experiment(self, c, stats, setuppkts=None):
        ...
        fpgas = [fpga for fpga in expt['FPGAs']]
        for fpga in fpgas:
            if fpga in expt['TimerStarted']:
                if fpga in expt['TimerStopped']:
                    if expt['TimerStopped'][fpga] < expt['TimerStarted'][fpga]:
        ...
        p.daisy_chain(fpgas)
        p.timing_order([self.getQubit(qname)['Timing'][1] for qname in self.Setups[expt['Setup']]['Qubits']])
